...
def resize_opencv(image, weight=10, height=10):
    smallImage = cv.resize(image, (weight, height), interpolation=cv.INTER_LANCZOS4)
    return smallImage


def calculateDifference(image, weight=10, height=10):
    differenceBuffer = []
...
def box_feature(img, box):
    # 根据传入的图像以及目标坐标，计算相似度hash值
    pic = stringToHash(img)
    return pic


def main():
    pic1 = stringToHash('D:\ProgramData\YOLO-V3-Tensorflow-dev\YOLO-V3-Tensorflow-dev\data\my_data\JPEGImages/0068.jpg')
    pic2 = stringToHash('D:\ProgramData\YOLO-V3-Tensorflow-dev\YOLO-V3-Tensorflow-dev\data\my_data\JPEGImages/0069.jpg')
    print(pic1)
    print(pic2)
